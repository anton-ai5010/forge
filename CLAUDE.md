...
`.forge/` (задачи, планы, идеи, решения, журнал) — **часть репозитория**, переживает смерть диска/VPS. Служебный мусор (`.inject-state`, `.last-backup`, `state.yml`, `.github-*`, `graph.json`, `status-report.html`, `reports/shots/`) отсечён через `.forge/.gitignore`. Автосохранение: `session-awareness` (итог сессии) и `finishing` (после мержа) зовут `skills/memory-backup/backup.sh` — коммит только `.forge`-путей + push текущей ветки. Ручной запуск: `/forge:save` или «сохрани». Напоминание при старте сессии, если несохранённое старше суток (session-start.sh). Без удалёнки — предложение приватного репозитория (только с явного «да»). Секреты в `.forge` не пишутся — только названия записей Bitwarden.
...
**Память (петля):** пишет `.forge/direction.yml` (для Клода: directions/backlog/goal_shift). Отдельный документ «для глаз» не заводит — это отчёт «Что дальше» (Phase 5). Цикл: разговор → память → навигатор → задачи → результаты → снова память.
...
Отчёт «Что дальше» — единственный документ «где мы» для владельца: в каком состоянии проект, что чинит Клод в коде, какие решения нужны от владельца, в каком порядке и почему. Формат-эталон — `forge-plugin/docs/status-report-format.md`. Полная сборка — по команде или словам «собери отчёт», «отчёт „что дальше“», «что чинить, что решать», «покажи документом, где мы»: аудит 4 параллельными субагентами (TODO/FIXME/заглушки, git-активность, инфраструктура, документация/`.forge`) + память `.forge` → находки с полями owner (Код / Решение / Решение+код), effort (S|M|L), block (критично / решения / скоро / потом), status (open|done|deferred) → `.forge/status-report.json` (память проекта, коммитится) → `skills/status-report/render.py` собирает `.forge/status-report.html` (Клод HTML руками не пишет; файл регенерируется, поэтому в `.forge/.gitignore`) → открывается в браузере. Снимки экрана — только если у проекта есть живой http-адрес (Playwright MCP, не больше 6, PNG в `.forge/reports/shots/`); нет адреса — отчёт без картинок, без вопросов. «Открой отчёт» / «покажи отчёт» — показывает уже собранный документ без нового аудита. Ответил на открытое решение в любой форме — Клод записывает его в `decisions.yml` и закрывает карточку. Лёгкое обновление — само после «мержим»: `finishing` зовёт `render.py merged <task-slug>` (карточка задачи → «Сделано», счётчик «устарел на N задач» +1, HTML пересобран). Карточка «Код», взятая в работу, уходит в `/forge:new-task`; после сохранения task-файла new-task связывает её с задачей (`render.py link <id> <slug>`). Слова «что дальше по проекту» остаются за unblocker, «статус проекта» — за загрузкой контекста. На GitHub отчёт не отражается.

**Выход:** `.forge/status-report.json` + `.forge/status-report.html`; при старте сессии — одна строка «ждут N решений владельца, отчёт устарел на M задач» (session-start.sh).
...
- `criteria/*.yml` — бинарные критерии по всем 7 фазам (unblocker, refine-idea и status-report добавлены), ни разу не применялись
