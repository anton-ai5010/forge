...
**Гайд по проекту (Фаза 5)** — если в проекте есть `.forge/guide/v*.json`, обнови его механически, без нового аудита, **до** сохранения памяти (обновлённые `.forge/guide/vX.Y.json` и `docs/guide/guide-latest.html` уедут тем же коммитом backup.sh):
```bash
python3 "$CLAUDE_PLUGIN_ROOT/skills/project-guide/render.py" merged "<task-slug>"
```
Риск этой задачи в «Карте рисков» (у него стояло «→ в работе») становится «→ уже работает», счётчик «гайд устарел на N задач» растёт на 1, `docs/guide/guide-vX.Y.html` и `guide-latest.html` перерисованы **без новой версии** (номер меняется только по слову «собери гайд»). Нет гайда — скрипт молчит (rc=0): ничего не делай и не упоминай.

`<task-slug>` — имя `<feature-branch>` без префикса `feat/` (или `fix/`): execute заводит ветку `feat/<slug>`, где slug = имя файла `.forge/tasks/YYYY-MM-DD-<slug>.md` без даты и `.md`. **Запомни имя ветки до `git checkout <base-branch>`** — после мержа её уже нет. Ветка названа иначе — возьми slug из имени task-файла этой задачи; **не** из `state.yml` (там уже `idle`) и **не** из `now.task`. Строка `FORGE-GUIDE: карточки с задачей «…» нет` — норма для задачи не из гайда, человеку об этом не говори.
