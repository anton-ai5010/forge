...
if [ -n "$(git status --porcelain)" ]; then
  ...
  git add -A
  # Память (.forge: задачи/планы/решения) коммитится — это ценность. Убираем только
  # runtime-мусор, если он исторически оказался под трекингом (.forge/.gitignore
  # не спасает уже-отслеживаемые файлы):
  git reset -q .forge/state.yml .forge/.inject-state .forge/.last-backup .forge/graph.json 2>/dev/null || true
  git reset -q -- .forge/.github-* 2>/dev/null || true
...
**Перед авто-коммитом** скажи человеку простой строкой что сохраняешь: *«Сохраняю эти файлы: …»* (перечисли из `git status --short`) — **это единственная защита**: показанный список и есть то, что уйдёт в коммит. (Runtime-мусор `.forge` выше явно убирается через `git reset` — `.gitignore` не спасает уже-отслеживаемые файлы; сама память `.forge` — задачи, планы, решения — наоборот, коммитится.) Текст коммита формируй сам из названия задачи, **на русском** — пользователь его не подтверждает, не грузим.
...
**Отчёт «Что дальше» (Фаза 5)** — если в проекте есть `.forge/status-report.json`, обнови его механически, без нового аудита, **до** сохранения памяти (обновлённый JSON уедет тем же коммитом backup.sh):
```bash
python3 "$CLAUDE_PLUGIN_ROOT/skills/status-report/render.py" merged "<task-slug>"
```
Карточка с этой задачей становится «Сделано», счётчик «устарел на N задач» растёт на 1, `.forge/status-report.html` пересобран. Нет отчёта — скрипт молчит (rc=0): ничего не делай и не упоминай. Полная пересборка с аудитом — только по слову «собери отчёт».

`<task-slug>` — имя `<feature-branch>` без префикса `feat/` (или `fix/`): execute заводит ветку `feat/<slug>`, где slug = имя файла `.forge/tasks/YYYY-MM-DD-<slug>.md` без даты и `.md`. **Запомни имя ветки до `git checkout <base-branch>`** — после мержа её уже нет. Ветка названа иначе — возьми slug из имени task-файла этой задачи; **не** из `state.yml` (там уже `idle`) и **не** из `now.task`. Строка `FORGE-REPORT: карточки с задачей «…» нет` — норма для задачи не из отчёта, человеку об этом не говори.

Владельцу — одна строка и только если в выводе есть `FORGE-REPORT: сделано → …`: добавь к подтверждению *«Карточка «<название>» в отчёте отмечена сделанной»*. Остальные строки `FORGE-REPORT:` — служебные, для тебя.

**Затем сохрани память и результат в облако** (тихо, по процедуре скилла `memory-backup`):
```bash
bash "$CLAUDE_PLUGIN_ROOT/skills/memory-backup/backup.sh" "<3-6 слов: что влили>"
```
Он закоммитит обновлённую память и запушит основную ветку (вместе с мержем) на удалёнку. Разбор его вывода — в SKILL.md memory-backup. Push не прошёл или удалёнки нет — одна строка человеку, работу не блокируй.
