...
description: "Use proactively when work produces a fact future sessions need — a decision was made, an approach failed twice, a step finished, a blocker appeared, a lesson surfaced, or the session is wrapping up. Russian voice cues: 'запиши решение', 'это дед энд / тупик', 'добавь в журнал', 'сделали Х / закончили Х', 'итог сессии', 'не повтори ошибку', 'зафиксируй', 'провалилось', 'не сработало', 'почему мы выбрали'. English: 'log this', 'record decision', 'mark as dead-end', 'session summary', 'remember for next time'. Writes to .forge/ files: index.yml session state (goal/now/done/next/errors/branch), dead-ends.yml after 2+ failed attempts, decisions.yml on architectural choices, status.yml on working/broken/blocked changes, journal.yml at session end. Reading is the hook's job — this skill is WRITE-only. Skip it and the next session repeats today's dead-ends, reverses today's decisions, redoes today's work from zero. Future Anton has no memory of today; these files are his only inheritance."
---
