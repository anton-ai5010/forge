...
description: "Use during plan/execute when the step touches database schema: add/remove tables or columns, change types/constraints, rename fields, backfill, ALTER TABLE, any migration (Postgres/MySQL/SQLite). RU: 'добавь поле в базу', 'добавить столбец', 'миграция', 'таблица в базе', 'схема бд', 'накати миграцию', 'откати базу', 'индекс на таблицу'. EN: 'add column', 'alter table', 'migration', 'schema change', 'backfill'. Also on edits to migrations/*, schema.sql, prisma/alembic/knex files. Every migration gets a reversibility plan BEFORE production."
---
