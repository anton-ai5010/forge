---
name: code-cleanup
description: "Use proactively when the user wants to clean up, tidy, refactor, simplify, or rehabilitate a codebase — even if they just grumble about mess. Triggers on Russian voice phrases 'почисти', 'причеши', 'разгреби', 'наведи порядок', 'упрости', 'выкинь мусор', 'говнокод', 'лапша', 'бардак в коде', 'технический долг', 'рефактор', and English 'clean up', 'refactor', 'lint', 'tidy', 'code quality', 'technical debt', 'dead code', 'messy code'. Also use when symptoms appear: dead code, unused imports, naming inconsistencies (camelCase vs snake_case mix), functions over 40 lines, nesting 3+ levels, copy-pasted logic, files doing too much, before a release/handoff, or after rapid prototyping. Runs parallel agents to scan, produces a structured severity report (high/medium/low), then fixes ONLY after explicit user approval — never silently rewrites. Skipping this means mess compounds: every new feature gets harder, bugs hide in duplication, and the team grows afraid to touch the code. One cleanup pass now saves weeks of confused debugging later."
---
