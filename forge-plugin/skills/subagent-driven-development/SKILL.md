...
description: "Use ONLY when the user explicitly asks for the subagent-driven double-review pattern, or when /execute delegates an independent step under strict review gates. Triggers: 'subagent-driven', 'двойное ревью', 'свежий агент на каждую задачу', 'двухступенчатое ревью', 'spec + quality review', 'делегируй с проверкой'. Runs a fresh subagent per task with TWO mandatory reviews (spec compliance, then code quality) and re-review loops until both approve. NOT the default path — /execute is."
---
...
digraph process {
    ...
    subgraph cluster_per_task {
        ...
        "Dispatch forge-documenter subagent (Sonnet)" [shape=box];
    ...
    "Code quality reviewer subagent approves?" -> "Dispatch forge-documenter subagent (Sonnet)" [label="yes"];
    "Dispatch forge-documenter subagent (Sonnet)" -> "Mark task complete in TodoWrite";
