---
name: api-design
description: "Use during plan/execute when the step involves an HTTP endpoint, REST route, or API contract — BEFORE writing handler code. RU: 'нужен api', 'сделай эндпоинт', 'ручка', 'роут', 'контракт api', 'GET /users', 'отдай json', 'связать с бэком'. EN: 'new endpoint', 'design API', 'REST route', 'API contract', 'add route'. Also for pagination choice, error shape, status codes. Contract-first: outputs a reviewable contract to .forge/plans/api-{name}.yml before any handler."
---
