...
description: "Use during plan/execute when the step touches auth, payments, user data, file uploads, API endpoints, or secrets. RU: 'безопасно ли', 'проверь безопасность', 'проверь на дыры', 'уязвимость', 'дыра в безопасности', 'утечка токена', 'хранение пароля', 'api ключ в коде', 'sql инжекция', 'xss', 'csrf', 'взлом'. EN: 'is this safe', 'security check', 'vulnerability', 'pentest', 'owasp'. Auto-trigger on login flows, JWT/sessions, payments, uploads, .env files, string-concat SQL, new public routes. Runs the security checklist before merge/deploy."
---
