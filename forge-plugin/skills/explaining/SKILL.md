---
name: explaining
description: "Use proactively when the user wants to understand how a piece of the project works — auth, payments, sync, any flow. RU: 'объясни как', 'покажи как работает', 'как устроен', 'как это работает', 'визуализируй', 'нарисуй', 'покажи поток'. EN: 'how does X work', 'explain how', 'visualize', 'walk me through'. Produces an interactive HTML page: краткий ответ + поток шагов + сущности + похожие вопросы. For deep understanding and visual learners, not quick yes/no answers. Explanation phase, not implementation."
---
