...
description: "Use proactively when about to claim work is done, fixed, working, or passing — BEFORE saying 'готово', 'закончил', 'сделал', 'работает', 'тесты прошли', 'done', 'ready', 'works', and BEFORE every commit, PR, or handoff. Also on 'проверь что работает', 'проверь результат', 'убедись'. Requires actually running the relevant command (tests, build, lint, the feature itself) and quoting output as evidence before any success claim. No 'should work', no 'looks fine' — run it, see it, then say it."
---
