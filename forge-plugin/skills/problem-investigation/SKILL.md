...
description: "Use proactively when the user signals confusion about behavior BEFORE asking for a fix — the pre-fix understanding phase. RU: 'почему', 'странно', 'непонятно', 'разберись', 'что происходит', 'не пойму', 'выясни', 'копни', 'чё за фигня', 'что-то не так'. EN: 'why', 'weird', 'investigate', 'diagnose'. Also when the user pastes an error/stack trace without asking to fix it. Maps symptom to root cause before touching code. Маршрут: непонятно ПОЧЕМУ → этот скилл; просят ПОЧИНИТЬ → systematic-debugging; новая фича → new-task."
---
