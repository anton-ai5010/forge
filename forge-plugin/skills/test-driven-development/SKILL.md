...
description: "Use during plan/execute when the step involves implementing a feature, bugfix, or new function — BEFORE writing implementation code. RU: 'напиши тест', 'тесты', 'покрытие', 'TDD', 'юнит', 'тест на', 'сначала тест'. EN: 'test', 'tdd', 'unit', 'spec', 'coverage', 'RED-GREEN', 'write a test'. Enforces RED-GREEN-REFACTOR: write a failing test first, watch it fail, minimal code to pass, then refactor. Tests are the specification — if you didn't watch it fail, you don't know what it tests."
---
