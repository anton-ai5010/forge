---
name: forge-context
description: "Use proactively at session start in any project with .forge/index.yml — loads project context via the tiered L0/L1/L2 system: L0 catalog auto-injected, L1 files routed by catalog tags, L2 specs only when an L1 summary is not enough. RU: 'начали сессию', 'загрузи контекст', 'что у нас в проекте', 'статус проекта', 'на чём остановились', 'продолжаем'. Also before any task needing project memory: conventions, dead-ends, resuming work, infra. Context-loader — distinct from using-forge (the general 'check skills first' guardrail)."
---
...
| Check infra/Docker/DB/server | docker, server, database, infra | `.forge/infrastructure.yml` |
