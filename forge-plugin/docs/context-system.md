...
```
L0 (~600 токенов для кириллического index.yml) — ВСЕГДА в контексте. Каталог всего что есть.
...
Загружается КАЖДЫЙ промпт через hook. Бюджет: ≤2500 байт (обрезка хуком); реальный кириллический index.yml по замеру — ~600 токенов.
...
# Читает ТОЛЬКО index файл (первые 2500 байт, ~600 токенов для кириллического)
