...
# Create skills in personal and forge locations for testing
mkdir -p "$TEST_HOME/personal-skills/shared-skill"
mkdir -p "$TEST_HOME/forge-skills/shared-skill"
mkdir -p "$TEST_HOME/forge-skills/unique-skill"
...
cat > "$TEST_HOME/forge-skills/shared-skill/SKILL.md" <<'EOF'
---
name: shared-skill
...
cat > "$TEST_HOME/forge-skills/unique-skill/SKILL.md" <<'EOF'
---
name: unique-skill
description: Only in forge
---
# Unique
EOF
...
function resolveSkillPath(skillName, forgeDir, personalDir) {
    const forceForge = skillName.startsWith('forge:');
    const actualSkillName = forceForge ? skillName.replace(/^forge:/, '') : skillName;

    if (!forceForge && personalDir) {
    ...
    if (forgeDir) {
        const forgePath = path.join(forgeDir, actualSkillName);
        const forgeSkillFile = path.join(forgePath, 'SKILL.md');
        if (fs.existsSync(forgeSkillFile)) {
            return {
                skillFile: forgeSkillFile,
                sourceType: 'forge',
                skillPath: actualSkillName
            };
        }
...
const forgeDir = '$TEST_HOME/forge-skills';
const personalDir = '$TEST_HOME/personal-skills';

// Test 1: Shared skill should resolve to personal
const shared = resolveSkillPath('shared-skill', forgeDir, personalDir);
console.log('SHARED:', JSON.stringify(shared));

// Test 2: forge: prefix should force forge
const forced = resolveSkillPath('forge:shared-skill', forgeDir, personalDir);
console.log('FORCED:', JSON.stringify(forced));

// Test 3: Unique skill should resolve to forge
const unique = resolveSkillPath('unique-skill', forgeDir, personalDir);
console.log('UNIQUE:', JSON.stringify(unique));

// Test 4: Non-existent skill
const notfound = resolveSkillPath('not-a-skill', forgeDir, personalDir);
console.log('NOTFOUND:', JSON.stringify(notfound));
" 2>&1)

if echo "$result" | grep -q 'SHARED:.*"sourceType":"personal"'; then
    echo "  [PASS] Personal skills shadow forge skills"
else
...
if echo "$result" | grep -q 'FORCED:.*"sourceType":"forge"'; then
    echo "  [PASS] forge: prefix forces forge resolution"
else
    echo "  [FAIL] forge: prefix not working"
    exit 1
fi

if echo "$result" | grep -q 'UNIQUE:.*"sourceType":"forge"'; then
    echo "  [PASS] Unique forge skills are found"
else
    echo "  [FAIL] Unique forge skills not found"
    exit 1
fi
