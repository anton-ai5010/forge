...
elif echo "$output" | grep -qi "PRIORITY_MARKER_FORGE_VERSION"; then
    echo "  [FAIL] Forge version loaded instead of personal"
    exit 1
else
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION"; then
    ...
    echo "  [FAIL] Forge version loaded instead of project"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
...
if echo "$output" | grep -qi "PRIORITY_MARKER_FORGE_VERSION"; then
