...
1. **Claude забывает контекст** — Forge создаёт `docs/index.md` (~400 токенов), который Claude читает перед каждым ответом и обновляет после каждого действия. Это внешняя рабочая память: даже если контекст сжимается, Claude перечитывает файл и восстанавливается.

2. **Claude повторяет ошибки** — Forge ведёт `docs/dead-ends/` с провальными подходами по темам. Перед любым фиксом Claude проверяет — не пробовали ли мы это уже? Записи создаются автоматически при неудачах.

3. **Claude не знает стадию проекта** — `index.md` содержит цель, фазу, прогресс, текущую задачу, результаты последней сессии. Открываете проект через неделю — Claude сразу знает где вы остановились.

Плюс Forge даёт структурированные процессы: от идеи через дизайн и план до готового кода с тестами. Вместо чтения всего кода (~40k+ токенов) Claude читает компактную документацию (~400-800 токенов).
...
```
docs/
├── index.md              # ТОЧКА ВХОДА: цель, стадия, текущая задача, live-сессия (~400 токенов)
...
- docs/status.md, decisions.md, journal.md
- docs/dead-ends/ (провальные подходы)
...
**Claude читает index.md и сразу в контексте:**
```
[Вы открываете проект через неделю]

Claude: [Читает docs/index.md → journal.md → проверяет dead-ends/]
...
Claude НЕ спрашивает "что делаем?" — он уже знает из index.md.

**Во время работы Claude обновляет index.md:**
```
[После каждого действия обновляется секция Session (live)]
[Если подход не сработал — записывается в dead-ends/]
[Если принято решение — записывается в decisions.md]
[Если контекст сжался — Claude перечитывает index.md и восстанавливается]
```
...
#### Hook: автоматический инжект контекста
На КАЖДЫЙ ваш запрос hook автоматически подставляет в промпт содержимое `docs/index.md`, список dead-ends тем, текущую ветку и последние коммиты. Claude получает контекст мгновенно, без tool calls. Работает только в проектах с `docs/index.md`.

**Когда:** Автоматически, 100% надёжность, нулевая задержка.

#### session-awareness
Отвечает за ЗАПИСЬ контекста: обновляет index.md после каждого действия, записывает провалы в dead-ends/, решения в decisions.md, состояние в status.md.

**Когда:** При завершении шагов, ошибках, провалах подходов, архитектурных решениях.

#### forge-context
Загружает полный контекст проекта из docs/ при старте. ~400 токенов index.md вместо 40k+ кода.

**Когда:** Автоматически при старте сессии если есть docs/index.md.
...
### docs/index.md — точка входа (читается ВСЕГДА)

```markdown
# Project: trading-bot
Автоматическая торговля на основе технического анализа
...
Modified: cache/redis_client.py, api/decorators.py
...
## Last Session
2026-02-14 — Исправил формат cache key, все тесты проходят
```

**Для чего:** Claude читает один файл (~400 токенов) и сразу знает: цель проекта, стадию, текущую задачу, что уже сделано в этой сессии. При сжатии контекста — перечитывает и восстанавливается.

### docs/status.md — что работает / сломано
...
### docs/dead-ends/ — провальные подходы по темам
...
Claude делает `ls docs/dead-ends/`, находит релевантный файл, читает ТОЛЬКО его.

**Для чего:** Claude не повторяет ошибки. Попробовали — не работает — записали — больше не пробуем.

### docs/journal.md — история сессий
...
```bash
ls docs/
# Должно быть: index.md, status.md, decisions.md, dead-ends/,
# journal.md, journal-archive/, map.json, conventions.json,
# plans/, library/
```

Если у проекта была старая структура (state.json, history.log) — Claude предложит миграцию.

### Шаг 2: Проверка контекста при старте сессии
...
В секции `## Session (live)` должно обновиться:
...
Claude должен перечитать `docs/index.md` и ответить по секции `Session (live)`, а не сказать "я не помню".

### Шаг 6: Проверка между сессиями

Закройте сессию, откройте новую в том же проекте. Claude должен:
1. Прочитать `index.md` — знать цель, стадию, последнюю сессию
2. Прочитать `journal.md` — знать что делали в прошлый раз
...
| Проблема | Проверить |
|---|---|
| Claude не знает стадию проекта | `cat docs/index.md` — есть ли Stage? |
| Claude повторяет ошибку | `ls docs/dead-ends/` — записана ли она? |
| Claude не помнит что делал | `cat docs/index.md` — обновляется ли Session? |
| Claude не знает решений | `cat docs/decisions.md` — записаны ли они? |
| Новая сессия без контекста | `cat docs/journal.md` — есть ли записи? |
...
Два механизма:
1. **Hook** — на каждый ваш запрос содержимое `docs/index.md` автоматически попадает в промпт. 100% надёжно — данные уже в промпте, невозможно пропустить.
2. **session-awareness** — обновляет `index.md` после каждого действия. При сжатии контекста hook при следующем запросе снова подставит актуальный index.md.

### Как Claude не повторяет ошибки?

Hook подставляет список тем из `docs/dead-ends/` в каждый запрос. При вызове скиллов (brainstorming, debugging) через backtick подгружается полное содержимое dead-ends. Записи создаются скиллом `session-awareness` при провалах.

### Что такое /forge:start?

Полная загрузка контекста в начале сессии — дашборд: цель, стадия, что работает/сломано, dead-ends, журнал. Запускать один раз при открытии проекта. Дальше hook поддерживает контекст автоматически.

### Нужно ли вручную обновлять документацию?

`session-awareness` обновляет `index.md`, `dead-ends/`, `decisions.md` во время работы. `/forge:sync` обновляет `library/`, `map.json`, `journal.md` — после завершения фичи.
