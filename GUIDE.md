...
#### session-awareness
Внешняя рабочая память Claude. Читает docs/index.md перед КАЖДЫМ ответом, обновляет его после каждого действия. Автоматически записывает провалы в dead-ends/, решения в decisions.md. При сжатии контекста — перечитывает index.md и восстанавливается.

**Когда:** Автоматически на КАЖДЫЙ запрос в проектах с docs/index.md.

#### forge-context
Загружает контекст проекта из docs/ вместо чтения всего кода. ~400 токенов index.md вместо 40k+ кода.
...
Через `docs/index.md` — файл обновляется после каждого действия (секция `Session (live)`). Если контекст сжимается в длинной сессии, Claude перечитывает этот файл и восстанавливает контекст. Это работает автоматически через скилл `session-awareness`.

### Как Claude не повторяет ошибки?

Через `docs/dead-ends/` — когда подход не сработал, Claude автоматически записывает это в файл по теме (например `dead-ends/auth.md`). Перед следующей попыткой он проверяет — не пробовали ли мы это уже. Записи накапливаются и работают между сессиями.

### Нужно ли вручную обновлять документацию?

`session-awareness` обновляет `index.md`, `dead-ends/`, `decisions.md` автоматически во время работы. `/forge:sync` обновляет `library/`, `map.json`, `journal.md` — его нужно запускать после завершения фичи.
